...
from Tiger.constants import mod_24_to_12
...
class CalendarGrid(object):
    ...
    def draw_grid(self):
        ...
        for i in range(hour_count + 1):
            text = mod_24_to_12[str(hour_from + i)]
    ...
    def _get_day_title(self, day: date) -> str:
        ...
        if not self.config.show_date:
            ...
            date_value = day.strftime('%m.%d') + (day.strftime('.%Y') if self.config.show_year else '')
            return f'{weekday}, {date_value}'
